...
function EditVideoModal({ video, onClose, onUpdate, onDelete }) {
    ...
    const handleSave = async (e) => {
        ...
        setError('');
        try {
            const response = await api.patch(`/librarian/edit-video/${video.id}`, formData);
    ...
    const handleDelete = async () => {
        if (window.confirm(`Are you sure you want to delete "${video.title}"? This action cannot be undone.`)) {
            try {
                await api.delete(`/librarian/delete-video/${video.id}`);
                onDelete(video.id);
                onClose();
            } catch (err) {
            ...
            <div className="edit-media-modal">
                ...
                <form onSubmit={handleSave} className="modal-form">
                    ...
                    <div className="form-grid">
                        ...
                        <div className="form-group"><label>Creator</label><input type="text" name="creator" value={formData.creator} onChange={handleChange} /></div>
