...
function ParentChildStatistics() {
  ...
  // 2️⃣ Fetch and compute stats + favorites for selected child
  useEffect(() => {
    ...
    const fetchChildStats = async () => {
      ...
      try {
        ...
        // --- compute today's & this week's favorites
        const now = new Date();
        ...
        const favoritesToday = favs.filter(f => {
          ...
          try { return new Date(f.created_at).toDateString() === todayStr; } catch { return false; }
        });

        const favoritesWeek = favs.filter(f => {
          if (!f.created_at) return false;
          try { return new Date(f.created_at) >= oneWeekAgo; } catch { return false; }
        });
        ...
        setStats({ totalFavoritesWeek: 0, totalFavoritesToday: 0, topCategory: 'N/A' });
        setLast3Favorites([]);
  ...
  if (error) {
    return <div className="parent-stats-container"><p style={{ color: 'red' }}>{error}</p></div>;
  }

  if (childrenList.length === 0) {
    return <div className="parent-stats-container"><p>No children linked to your account.</p></div>;
  }
    ...
    <div className="parent-stats-container">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* 🕮 Last 3 favorites for selected child */}
            ...
            {last3Favorites.length > 0 ? (
              <ul>
                {last3Favorites.map((f) => (
                  <li key={f.id}>
                    {f.type?.toLowerCase() === 'book' ? '📘' : '🎬'} <strong>{f.title}</strong>
                    {f.created_at ? <span> · {new Date(f.created_at).toLocaleString()}</span> : null}
                  </li>
                ))}
              </ul>
            ) : (
              ...
              <ul>
                {favoritesForSelected.map(f => (
                  <li key={f.id}>
                    ...
                    {f.created_at ? <span> · {new Date(f.created_at).toLocaleDateString()}</span> : null}
                  </li>
                ))}
              </ul>
            ) : (
