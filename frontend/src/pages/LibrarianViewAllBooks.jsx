...
function ViewAllBooks() {
  ...
  // NEW: keep this scoped to books only
  ...
  // Fetch BOOK sources only
  useEffect(() => {
    const fetchSources = async () => {
      setSourcesLoading(true);
      try {
        const { data } = await api.get('/librarian/book-sources');
  ...
  const fetchBooks = useCallback(async () => {
    ...
    try {
      const params = {
        ...
        // only send source when selected
        source: sourceFilter || undefined,
      };
      const { data } = await api.get('/librarian/view-all-books', { params });
      setBooks(data.items || []);
              ...
              <tbody>
                {books.map((book) => (
                  <tr key={book.id}>
                    ...
                    <td>{book.source || '—'}</td>
                    <td>{typeof book.rating === 'number' ? book.rating.toFixed(1) : '—'}</td>
                    <td>
